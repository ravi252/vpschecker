...
async def _run_check(update, context, input_path, status_msg, run_dir):
    ...
    unreachable = [r for r in all_results if r.get("status") == "UNREACHABLE"]
    auth_failed = [r for r in all_results if r.get("status") == "AUTH_FAILED"]
    errors      = [r for r in all_results if r.get("status") == "ERROR"]
    skipped     = [r for r in all_results if r.get("status") == "SKIPPED"]
    failed      = unreachable + auth_failed + errors
    wpath, dpath = run_dir / "working.txt", run_dir / "dead.txt"
    write_working(wpath, working)
    write_dead(dpath, failed)
    ...
    note = ""
    if skipped:
        note = (f"\n⏭ {len(skipped)} server(s) were NOT checked (time limit). "
    ...
    if failed:
        lines = []
        for r in failed[:20]:
            lines.append(f"• {r['ip']}:{r['port']}:{r['user']} → {r.get('status')} ({r.get('detail', '?')})")
        if len(failed) > 20:
            lines.append(f"• ...and {len(failed) - 20} more (see dead.txt)")
        dead_details = "\n\n🔍 Details:\n" + "\n".join(lines)
    ...
    await safe_edit(
        ...
        f"🔴 Dead      : {len(unreachable)}\n"
        f"🔑 Auth fail : {len(auth_failed)}\n"
        f"⚠️ Errors    : {len(errors)}{note}{dead_details}\n\n📎 Sending working.txt + the list below…")
...
def main():
    ...
    builder = builder.request(HTTPXRequest(
        read_timeout=120.0, write_timeout=120.0, connect_timeout=12.0))
    if LOCAL_API_URL:
        builder = (builder
