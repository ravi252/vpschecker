...
def check_vps(vps: Vps, timeout: float, ping_only: bool,
    ...
    if not reachable and _remaining() > 0:
        # transient network blip -> retry once before writing it off as dead
        reachable, reason = _tcp_reachable(vps.ip, vps.port,
                                           min(timeout, _remaining()))
    if not reachable:
